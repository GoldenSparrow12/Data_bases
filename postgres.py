...
def Check_table():
    ...
    record = cur.fetchall()
    for row in record:
         lst_table.append(list(row)[1])
    return lst_table

def Config():
    conn =  psycopg2.connect(
        database = 'mydatabase', user='codetrade', password='password', host = 'localhost' ,port = '5432'
    )
    return conn
...
def Create():
    ...
    if table_name in lst_table:
        print("Table Already Created!!") 
    
    else:
        try:
            cur = conn.cursor()
            cur.execute(f'''CREATE TABLE {table_name} (StudentID SERIAL PRIMARY KEY NOT NULL, 
            StudentName VARCHAR (20), StudentMark REAL);''')
            conn.commit()
